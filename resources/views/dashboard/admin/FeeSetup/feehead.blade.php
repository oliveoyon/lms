                        ...
                        <div class="card-header bg-navy">
                            ...
                            <div class="card-tools">
                                <ul class="nav nav-pills ml-auto">
                                    <li class="nav-item">
                                        <button class="btn btn-success btn-sm" data-toggle="modal" data-target="#addFeeHeadModal">
                        ...
                        <div class="card-body table-responsive">
                            ...
                            <table class="table table-bordered table-striped table-hover table-sm" id="fee-heads-table">
                                <thead style="border-top: 1px solid #b4b4b4">
                                    ...
                                    <th>{{ __('language.fee_head_frequency') }}</th>
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td class="{{ $feeHead->status == 1 ? 'text-success' : 'text-danger' }} font-weight-bold">
                                            {{ $feeHead->status == 1 ? 'Active' : 'Inactive' }}
                                        </td>
                                        <td>
                                            <div class="btn-group">
                                                <button type="button" class="btn btn-warning btn-xs" data-id="{{ $feeHead->id }}" id="editFeeHeadBtn">
                                                    <i class="fas fa-edit"></i>
                                                </button>
                                                <button type="button" class="btn btn-danger btn-xs" data-id="{{ $feeHead->id }}" id="deleteFeeHeadBtn">
    ...
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="{{ route('admin.addAcademicFeeHead') }}" method="POST" autocomplete="off" id="add-fee-head-form">
                    @csrf
                    <div class="row">
                        <div class="col-md-6">
                            ...
                            <div class="form-group">
                                <label for="aca_feehead_freq">{{ __('language.fee_head_frequency') }}</label>
...
<div class="modal fade editFeeHead" tabindex="-1" role="dialog" aria-labelledby="editFeeHeadLabel" aria-hidden="true" data-keyboard="false" data-backdrop="static">
    <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="{{ route('admin.updateAcademicFeeHeadDetails') }}" method="post" autocomplete="off" id="update-fee-head-form">
                    @csrf
...
<script>
    ...
    $('#update-fee-head-form').on('submit', function (e) {
        ...
        $(document).on('click', '#editFeeHeadBtn', function () {
            ...
            $.post("{{ route('admin.getAcademicFeeHeadDetails') }}", { academic_feehead_id: fee_head_id }, function (data) {
                $('.editFeeHead').find('input[name="fee_head_id"]').val(data.details.id);
                $('.editFeeHead').find('input[name="aca_feehead_name"]').val(data.details.aca_feehead_name);
                $('.editFeeHead').find('input[name="aca_feehead_description"]').val(data.details.aca_feehead_description);
                $('.editFeeHead').find('select[name="aca_feehead_freq"]').val(data.details.aca_feehead_freq);
                $('.editFeeHead').find('input[name="no_of_installment"]').val(data.details.no_of_installment);
    ...
    $(document).on('click', '#deleteFeeHeadBtn', function () {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.post(url, { academic_feehead_id: feeHeadId }, function (data) {
                    if (data.code == 1) {
                        var redirectUrl = data.redirect;
                        toastr.success(data.msg);
                        setTimeout(function () {
    ...
    // Open the edit modal for Academic Fee Head
    $(document).on('click', '#editFeeHeadBtn', function () {
        var feeHeadId = $(this).data('id');
        $('.editFeeHead').find('form')[0].reset();
        $('.editFeeHead').find('span.error-text').text('');
        $.post("{{ route('admin.getAcademicFeeHeadDetails') }}", { academic_feehead_id: feeHeadId }, function (data) {
            $('.editFeeHead').find('input[name="fee_head_id"]').val(data.details.id);
            $('.editFeeHead').find('input[name="aca_feehead_name"]').val(data.details.aca_feehead_name);
            $('.editFeeHead').find('input[name="aca_feehead_description"]').val(data.details.aca_feehead_description);
            $('.editFeeHead').find('select[name="aca_feehead_freq"]').val(data.details.aca_feehead_freq);
