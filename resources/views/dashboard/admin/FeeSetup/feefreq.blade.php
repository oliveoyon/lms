        ...
        <div class="row mb-2">
          ...
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              <li class="breadcrumb-item"><a href="{{ route('admin.home') }}">{{ __('language.fee_frequency') }}</a></li>
            </ol>
          </div><!-- /.col -->
        </div><!-- /.row -->
      </div><!-- /.container-fluid -->
                    ...
                    <div class="card-header bg-navy">
                        ...
                        <div class="card-tools">
                            <ul class="nav nav-pills ml-auto">
                                <li class="nav-item">
                                    <button class="btn btn-success btn-sm" data-toggle="modal" data-target="#addFeeFrequencyModal">
                                        <i class="fas fa-plus-square mr-1"></i> {{ __('language.fee_frequency_add') }}
                                    </button>
                                </li>
                            </ul>
                        </div>
                        ...
                        <table class="table table-bordered table-striped table-hover table-sm" id="fee-frequencies-table">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td class="{{ $feeFrequency->freq_status == 1 ? 'text-success' : 'text-danger' }} font-weight-bold">
                                        {{ $feeFrequency->freq_status == 1 ? 'Active' : 'Inactive' }}
                                    </td>
                                    <td>
                                        <div class="btn-group">
                                            <button type="button" class="btn btn-warning btn-xs" data-id="{{ $feeFrequency->id }}" id="editFeeFrequencyBtn">
                                                <i class="fas fa-edit"></i>
                                            </button>
                                            <button type="button" class="btn btn-danger btn-xs" data-id="{{ $feeFrequency->id }}" id="deleteFeeFrequencyBtn">
...
<!-- {{ __('language.fee_frequency_add') }} Modal -->
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control form-control-sm" name="freq_name" id="freq_name" placeholder="{{ __('language.fee_frequency_name') }}">
                                <span class="text-danger error-text freq_name_error"></span>
                            </div>
                            <div class="form-group">
                                <label for="no_of_installment">{{ __('language.no_of_installment') }}</label>
                                <input type="number" class="form-control form-control-sm" name="no_of_installment" id="no_of_installment" placeholder="{{ __('language.no_of_installment') }}">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control form-control-sm" name="installment_period" id="installment_period" placeholder="Installment Period">
...
<div class="modal fade editFeeFrequency" tabindex="-1" role="dialog" aria-labelledby="editFeeFrequencyLabel" aria-hidden="true" data-keyboard="false" data-backdrop="static">
    <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="editFeeFrequencyLabel">{{ __('language.fee_frequency_edit') }}</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <form action="{{ route('admin.updateFeeFrequencyDetails') }}" method="post" autocomplete="off" id="update-fee-frequency-form">
                    @csrf
...
<script>
    ...
    $(document).ready(function () {
        ...
        $(document).on('click', '#editFeeFrequencyBtn', function () {
            ...
            $.post("{{ route('admin.getFeeFrequencyDetails') }}", { fee_frequency_id: fee_frequency_id }, function (data) {
                $('.editFeeFrequency').find('input[name="fee_frequency_id"]').val(data.details.id);
                $('.editFeeFrequency').find('input[name="freq_name"]').val(data.details.freq_name);
                $('.editFeeFrequency').find('input[name="no_of_installment"]').val(data.details.no_of_installment);
                $('.editFeeFrequency').find('input[name="installment_period"]').val(data.details.installment_period);
                $('.editFeeFrequency').find('select[name="freq_status"]').val(data.details.freq_status);
        ...
        $(document).on('click', '#deleteFeeFrequencyBtn', function () {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.post(url, { fee_frequency_id: fee_frequency_id }, function (data) {
                        if (data.code == 1) {
                            var redirectUrl = data.redirect;
                            toastr.success(data.msg);
                            setTimeout(function () {
